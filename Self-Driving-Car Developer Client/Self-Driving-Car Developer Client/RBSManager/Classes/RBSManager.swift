                ...
                if let messageData = data["msg"] as? [String : Any] {
                    ...
                    if let message = messageType.init(map: map) {
                        DispatchQueue.main.async {
                            if subscriber.messageType == "sensor_msgs/Image"{
                                let decodedImgData = NSData(base64Encoded: messageData["data"] as! String, options: .ignoreUnknownCharacters)
                                let bytes = [UInt8](decodedImgData! as Data)
                                (message as! ImageMessage).data = bytes
                            }
